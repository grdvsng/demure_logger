...
from demure_logger.utils.list  import indexof
...
TEST_SORTED_FUNC      = lambda column : indexof( TEST_SORTED_ITER, column.name )
...
class TestMessage( unittest.TestCase ):
    ...
    def test_create_line_with_sorted_kwargs( self ):
        ...
        print( [ i for i in instance if isinstance( i.value, int ) ], [ getattr( instance, i ).value for i in TEST_SORTED_ITER ] )
        
        self.assertListEqual( [ i for i in instance if isinstance( i.value, int ) ], [ getattr( instance, i ).value for i in TEST_SORTED_ITER ] )
    ...
    def test_create_simple_instance( self ):
        ...
        self.assertListEqual( [ i for i in instance if isinstance( i.value, int ) ], [ TEST_COLUMN_1_VALUE, TEST_COLUMN_2_VALUE ] )
